...
export default function ActionCards() {
    ...
    const computeActionCards = (statuses: ProcessStatuses, events: Event[], suggestions: Suggestion[]) => {
        const actionsCards = [];
        ...
        if (upcomingEventsWithoutTimeOrLink > 0) {
            actionsCards.push(<ActionCard
                type="warn"
                title="Warning"
                description={<div>{upcomingEventsWithoutTimeOrLink} upcoming event(s) without time or watch link</div>}
    ...
    useEffect(() => {
        if (!!loadedStatuses && !!events && !!suggestions) {
            // setStatuses(loadedStatuses);
            computeActionCards(loadedStatuses, events, suggestions);
        }
    }, [loadedStatuses, events, suggestions])
    ...
    return (<div
        ...
        <div className="flex items-center gap-x-2 w-full">
            <div className={clsx('w-fit p-3 rounded-4xl',
                ...
                {type == 'error' && <ExclamationCircleIcon className="w-10"/>}
                {type == 'warn' && <ExclamationTriangleIcon className="w-10"/>}
                {type == 'info' && <InformationCircleIcon className="w-10"/>}
            </div>
